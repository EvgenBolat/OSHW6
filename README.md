...
В папке 10 расположены файлы, реализующие сервер и клиент, причём совместно завершающиеся. Для этого дополнительно были добавлены участки кода, позволяющие серверу и клиенту получить id потока-собеседника, и при нажатии ctrc+c в одном из них вызовется функция ExitProgram, которая внутри передаст потоку-собеседнику команду SIGINt, которая обработается уже в его функции ExitProgram. Тем самым, программы завершаются совместно.
